...
void updateGrid(struct records *g) { // use this to update grid after every move
	char grid[3][3];
	pair currentPair;
	for (int i = 0; i < 3; i++) {
		for (int j = 0; j < 3; j++ {
			...
			if (isElement(g->r, currentPair) == TRUE) {
				if (isElement(g->s, currentPair) == TRUE) {
					grid[i][j] = 'R'; // uppercase R if already marked
				} else {
					grid[i][j] = 'r'; // lowercase r if not marked
				}
			} else if (isElement(g->b, currentPair) == TRUE) {
				if (isElement(g->s, currentPair) == TRUE) {
...
void movePrompt(struct records *g) {
	updateGrid(g);
	pair move = {1, 1};
	if (go == TRUE) {
		// ask for x
